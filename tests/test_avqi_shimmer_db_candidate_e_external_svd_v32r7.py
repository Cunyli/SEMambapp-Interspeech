...
import scripts.evaluate_avqi_shimmer_db_candidate_e_external_svd_v32r7 as v32
...
def test_v32r7_runtime_config_freezes_science_and_binds_v32r6_no_go() -> None:
    path = (
        Path(v32.__file__).resolve().parents[1]
        / "configs"
        / "avqi_route_c_shimmer_db_candidate_e_external_svd_runtime_v32r7.json"
    )
    config = json.loads(path.read_text(encoding="utf-8"))
    assert config["schema_version"] == v32.RUNTIME_CONFIG_SCHEMA
    predecessor = config["predecessor_no_go"]
    assert predecessor["job_id"] == "20043947"
    assert predecessor["scientific_decision"] == v32.v32r6.PREEXACT_NO_GO
    assert predecessor["candidate_exact_outcomes_opened"] is False
    evidence = config["result_blind_runtime_evidence"]
    assert evidence["failed_case_count"] == 1
...
def test_v32r7_candidate_layout_refreshes_only_eight_nonzero_waveforms() -> None:
...
def test_v32r7_batched_proxy_matches_frozen_serial_cpu() -> None:
    sample_count = 16_000
...
def test_v32r7_certificate_selector_matches_frozen_selector(
...
def test_v32r7_prepared_pcm24_is_serially_written_byte_identical(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_v32r7_runtime_successor_rejects_predecessor_exact_opening(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config_path = (
        Path(v32.__file__).resolve().parents[1]
    ...
    runtime_by_case = {
        f"case-{index}": {
            ...
            "candidate_proxy_reduction_after_topology_ms": 10.0,
    ...
    report = {
        "schema_version": v32.v32r6.REPORT_SCHEMA,
        "decision": v32.v32r6.PREEXACT_NO_GO,
    ...
    receipt = {
        "schema_version": v32.v32r6.RECEIPT_SCHEMA,
        "decision": v32.v32r6.PREEXACT_NO_GO,
        "artifact_sha256": {
            "external_svd_report_v32r6.json": predecessor["report_sha256"],
    ...
    args = SimpleNamespace(
        ...
        v32r6_report=Path("report"),
        v32r6_report_sha256=predecessor["report_sha256"],
        v32r6_receipt=Path("receipt"),
        v32r6_receipt_sha256=predecessor["receipt_sha256"],
    )
    validated, observed = v32.validate_runtime_successor(args)
    assert validated is config
    assert observed["v32r6_report"] == predecessor["report_sha256"]
...
def test_v32r7_source_seals_selector_before_exact_and_never_trains() -> None:
...
def test_v32r7_source_overlaps_result_blind_pipeline_and_keeps_oracle() -> None:
...
def test_v32r7_promotion_receipt_keeps_training_closed(tmp_path: Path) -> None:
    artifact = tmp_path / "external_svd_report_v32r7.json"
    artifact.write_text("{}\n", encoding="utf-8")
    args = SimpleNamespace(
        output_dir=tmp_path,
        source_commit="a" * 40,
