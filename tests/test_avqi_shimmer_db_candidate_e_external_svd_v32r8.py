...
import scripts.evaluate_avqi_shimmer_db_candidate_e_external_svd_v32r8 as v32
...
def test_v32r8_runtime_config_freezes_science_and_binds_v32r7_no_go() -> None:
    path = (
        Path(v32.__file__).resolve().parents[1]
        / "configs"
        / "avqi_route_c_shimmer_db_candidate_e_external_svd_runtime_v32r8.json"
    )
    config = json.loads(path.read_text(encoding="utf-8"))
    assert config["schema_version"] == v32.RUNTIME_CONFIG_SCHEMA
    predecessor = config["predecessor_no_go"]
    assert predecessor["job_id"] == "20044276"
    assert predecessor["scientific_decision"] == v32.v32r7.PREEXACT_NO_GO
    assert predecessor["candidate_exact_outcomes_opened"] is False
    evidence = config["result_blind_runtime_evidence"]
    assert evidence["failed_case_count"] == 2
    assert evidence["candidate_exact_outcomes_used"] is False
    ...
    assert runtime["unique_pcm24_preparation_count"] == 9
    assert runtime["zero_step_preparation_deduplicated_by_pcm24_identity"] is True
...
def test_v32r8_batched_proxy_matches_frozen_serial_cpu() -> None:
    sample_count = 16_000
...
def test_v32r8_certificate_selector_matches_frozen_selector(
...
def test_v32r8_prepared_pcm24_is_serially_written_byte_identical(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_v32r8_runtime_successor_rejects_predecessor_exact_opening(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config_path = (
        Path(v32.__file__).resolve().parents[1]
    ...
    runtime_by_case = {
        f"case-{index}": {
            "total_metric_step_runtime_ms": (
                550.0 if index < predecessor["failed_case_count"] else 300.0
            ),
            "base_topology_refresh_ms": 100.0,
            "candidate_topology_and_metric_overlap_ms": 200.0,
            "candidate_unique_pcm24_count": 9.0,
        }
    ...
    runtime_by_case["case-0"]["base_topology_refresh_ms"] = config[
        "result_blind_runtime_evidence"
    ]["maximum_case_base_topology_refresh_ms"]
    runtime_by_case["case-0"][
        "candidate_topology_and_metric_overlap_ms"
    ] = config["result_blind_runtime_evidence"][
        "maximum_case_candidate_topology_and_metric_overlap_ms"
    ]
    report = {
        "schema_version": v32.v32r7.REPORT_SCHEMA,
        "decision": v32.v32r7.PREEXACT_NO_GO,
    ...
    receipt = {
        "schema_version": v32.v32r7.RECEIPT_SCHEMA,
        "decision": v32.v32r7.PREEXACT_NO_GO,
        "artifact_sha256": {
            "external_svd_report_v32r7.json": predecessor["report_sha256"],
    ...
    args = SimpleNamespace(
        ...
        v32r7_report=Path("report"),
        v32r7_report_sha256=predecessor["report_sha256"],
        v32r7_receipt=Path("receipt"),
        v32r7_receipt_sha256=predecessor["receipt_sha256"],
    )
    validated, observed = v32.validate_runtime_successor(args)
    assert validated is config
    assert observed["v32r7_report"] == predecessor["report_sha256"]
...
def test_v32r8_source_seals_selector_before_exact_and_never_trains() -> None:
    ...
    assert source.index("physical_cpu_allocation_certificate()") < source.index(
...
def test_v32r8_source_overlaps_result_blind_pipeline_and_keeps_oracle() -> None:
    ...
    assert "torch.stack" in proxy_source
    assert proxy_source.count(".cpu()") == 1
    assert "v29.build_candidate_pool" in main_source
    assert "compare_candidate_pools" in main_source


def test_v32r8_promotion_receipt_keeps_training_closed(tmp_path: Path) -> None:
    artifact = tmp_path / "external_svd_report_v32r8.json"
    artifact.write_text("{}\n", encoding="utf-8")
    args = SimpleNamespace(
        output_dir=tmp_path,
        source_commit="a" * 40,
    ...
    assert receipt["v32r6_preexact_no_go_preserved"] is True
    assert receipt["v32r7_preexact_no_go_preserved"] is True
