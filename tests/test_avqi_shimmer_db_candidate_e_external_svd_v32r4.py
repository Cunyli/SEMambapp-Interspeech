...
import scripts.evaluate_avqi_shimmer_db_candidate_e_external_svd_v32r4 as v32
...
def test_v32r4_runtime_config_freezes_science_and_binds_v32r3_no_go() -> None:
    path = (
        Path(v32.__file__).resolve().parents[1]
        / "configs"
        / "avqi_route_c_shimmer_db_candidate_e_external_svd_runtime_v32r4.json"
    )
    config = json.loads(path.read_text(encoding="utf-8"))
    assert config["schema_version"] == v32.RUNTIME_CONFIG_SCHEMA
    predecessor = config["predecessor_no_go"]
    assert predecessor["job_id"] == "20042036"
    assert predecessor["scientific_decision"] == v32.v32r3.PREEXACT_NO_GO
    assert predecessor["candidate_exact_outcomes_opened"] is False
    runtime = config["runtime_successor"]
    assert runtime["topology_worker_count"] == 9
    assert runtime["executor_worker_count"] == 10
    assert runtime["unique_pcm24_topology_refresh"] is True
...
def test_v32r4_candidate_layout_refreshes_nine_unique_pcm24_waveforms() -> None:
    unique, projected_zero, raw_zero = v32.candidate_layout()
    assert len(unique) == 9
    assert len(set(unique)) == 9
    assert projected_zero == 0
    assert raw_zero == 5
    assert raw_zero not in unique


def test_v32r4_batched_proxy_matches_frozen_serial_cpu() -> None:
    sample_count = 16_000
...
def test_v32r4_certificate_selector_matches_frozen_selector(
...
def test_v32r4_runtime_successor_rejects_predecessor_exact_opening(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config_path = (
        Path(v32.__file__).resolve().parents[1]
    ...
    report = {
        "schema_version": v32.v32r3.REPORT_SCHEMA,
        "decision": v32.v32r3.PREEXACT_NO_GO,
    ...
    receipt = {
        "schema_version": v32.v32r3.RECEIPT_SCHEMA,
        "decision": v32.v32r3.PREEXACT_NO_GO,
        "artifact_sha256": {
            "external_svd_report_v32r3.json": predecessor["report_sha256"],
    ...
    args = SimpleNamespace(
        ...
        v32r3_report=Path("report"),
        v32r3_report_sha256=predecessor["report_sha256"],
        v32r3_receipt=Path("receipt"),
        v32r3_receipt_sha256=predecessor["receipt_sha256"],
    )
    validated, observed = v32.validate_runtime_successor(args)
    assert validated is config
    assert observed["v32r3_report"] == predecessor["report_sha256"]
...
def test_v32r4_source_seals_selector_before_exact_and_never_trains() -> None:
...
def test_v32r4_source_pipelines_unique_topology_and_keeps_serial_oracle() -> None:
    pool_source = inspect.getsource(v32.build_candidate_pool_pipeline)
    main_source = inspect.getsource(v32.main)
    assert "materialize_and_refresh_candidate" in pool_source
...
def test_v32r4_promotion_receipt_keeps_training_closed(tmp_path: Path) -> None:
    artifact = tmp_path / "external_svd_report_v32r4.json"
    artifact.write_text("{}\n", encoding="utf-8")
    args = SimpleNamespace(
        output_dir=tmp_path,
        source_commit="a" * 40,
