...
EXTERNAL_RESERVE_UTTERANCES="${EXTERNAL_RESERVE_UTTERANCES:-2}"
EXPECTED_VCTK_CANDIDATE_ROWS="${EXPECTED_VCTK_CANDIDATE_ROWS:-1824}"
...
export MAX_OPEN_SHARDS EXTERNAL_RESERVE_UTTERANCES EXPECTED_VCTK_CANDIDATE_ROWS
...
conda run -n semambapp python scripts/prepare_avqi_component_v4_vctk.py \
  ...
  --output-dir "$DATA_DIR" \
  --seed "$SEED" \
  --external-reserve-utterances-per-speaker "$EXTERNAL_RESERVE_UTTERANCES" \
...
"$EXACT_PYTHON" scripts/build_avqi_component_v4_label_bank.py \
  ...
  --expected-vctk-rows "$EXPECTED_VCTK_CANDIDATE_ROWS" \
