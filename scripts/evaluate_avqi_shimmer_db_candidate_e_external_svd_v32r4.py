#!/usr/bin/env python3
"""Run the frozen Candidate-E external SVD adjudication with pipelined runtime.

V32r4 is a pure pre-exact implementation successor of v32r3.  It preserves
...
REPORT_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-exact-promotion-v32r4"
)
RECEIPT_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-receipt-v32r4"
)
SELECTOR_SEAL_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-selector-seal-v32r4"
)
PASS_DECISION = "PASS_CANDIDATE_E_EXACT_PRAAT_EXTERNAL_SVD_PROMOTION_V32R4"
PREEXACT_NO_GO = "NO_GO_CANDIDATE_E_EXTERNAL_SVD_PREEXACT_V32R4"
EXACT_NO_GO = "NO_GO_CANDIDATE_E_EXTERNAL_SVD_EXACT_PROMOTION_V32R4"
...
RUNTIME_CONFIG_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-runtime-successor-v32r4"
)
TOPOLOGY_WORKER_COUNT = 9
...
def repository_provenance(args: argparse.Namespace) -> dict[str, str]:
    ...
    if root != Path(__file__).resolve().parents[1]:
        raise ValueError("repository root does not contain v32r4 runner")
    head = v23.git_output(root, "rev-parse", "HEAD")
    if head != args.source_commit:
        raise ValueError("v32r4 repository HEAD/source commit drift")
    ...
    if status:
        raise ValueError("v32r4 adjudication requires a clean repository")
    return {
...
def validate_runtime_successor(
    ...
    observed = {
        "runtime_config": v23.validate_hash(
            ...
            "v32r4 runtime successor config",
        ),
        "v32r3_report": v23.validate_hash(
            args.v32r3_report,
            args.v32r3_report_sha256,
            "v32r3 pre-exact NO_GO report",
        ),
        "v32r3_receipt": v23.validate_hash(
            args.v32r3_receipt,
            args.v32r3_receipt_sha256,
            "v32r3 pre-exact NO_GO receipt",
        ),
    }
    config = v23.read_json(args.runtime_config)
    report = v23.read_json(args.v32r3_report)
    receipt = v23.read_json(args.v32r3_receipt)
    if config.get("schema_version") != RUNTIME_CONFIG_SCHEMA:
        raise ValueError("v32r4 runtime config schema drift")
    predecessor = config.get("predecessor_no_go", {})
    expected_predecessor = {
        "job_id": "20042036",
        "state": "COMPLETED",
        "exit_code": "0:0",
        "scientific_decision": v32r3.PREEXACT_NO_GO,
        "source_commit": "d5f6d448c7de5578c3b12164ba20ecb212e63ee3",
        "report_sha256": observed["v32r3_report"],
        "receipt_sha256": observed["v32r3_receipt"],
        "selector_seal_sha256": (
            "c29876f2cc3abf998fe7e88ebbab412d8ef1afc8f675905809295ea90d5400b8"
        ),
        "candidate_attempts_sha256": (
            "81db7a506143a19ae0d685b731ab01f5e9391755bd9328eb75f040b3e9bd6294"
        ),
        "candidate_pool_equivalence_sha256": (
            "a2d6cfc5dbff0e1d0eed74b2daa119b247fbd59fb583abcf2989918991f2a4a6"
        ),
        "stdout_sha256": (
            "40c809a7a848ef29c5e501f46432b7f8525635ef26b8f1e177a01194bc1b862a"
        ),
        "stderr_sha256": (
        ...
        "failed_case_count": 6,
        "maximum_total_metric_step_runtime_ms": 941.818253020756,
        "old_receipt_remains_immutable": True,
        "no_go_not_reinterpreted_as_pass": True,
    }
    if predecessor != expected_predecessor:
        raise ValueError("v32r4 predecessor NO_GO binding drift")
    if report.get("schema_version") != v32r3.REPORT_SCHEMA:
        raise ValueError("v32r3 predecessor report schema drift")
    if report.get("decision") != v32r3.PREEXACT_NO_GO:
        raise ValueError("v32r3 predecessor is not pre-exact NO_GO")
    if report.get("source_commit") != predecessor["source_commit"]:
        raise ValueError("v32r3 predecessor source binding drift")
    if report.get("slurm_job_id") != predecessor["job_id"]:
        raise ValueError("v32r3 predecessor job binding drift")
    expected_gates = {
    ...
    if report.get("preexact_gates") != expected_gates:
        raise ValueError("v32r3 predecessor failure-class drift")
    ...
    if (
        ...
        raise ValueError("v32r3 predecessor runtime evidence drift")
    if (
        ...
        raise ValueError("v32r3 predecessor opened or promoted exact outcomes")
    if receipt.get("schema_version") != v32r3.RECEIPT_SCHEMA:
        raise ValueError("v32r3 predecessor receipt schema drift")
    if receipt.get("decision") != v32r3.PREEXACT_NO_GO:
        raise ValueError("v32r3 predecessor receipt decision drift")
    expected_artifacts = {
        "external_svd_report_v32r3.json": observed["v32r3_report"],
        "selector_seal_pre_exact_v32r3.json": predecessor[
            "selector_seal_sha256"
        ],
        "candidate_e_attempts_pre_exact_v32r3.csv": predecessor[
            "candidate_attempts_sha256"
        ],
        "candidate_pool_equivalence_v32r3.json": predecessor[
            "candidate_pool_equivalence_sha256"
        ],
    }
    if receipt.get("artifact_sha256") != expected_artifacts:
        raise ValueError("v32r3 predecessor receipt artifact binding drift")
    if (
        ...
        raise ValueError("v32r3 predecessor receipt exact-opening drift")
    for label, value in (("report", report), ("receipt", receipt)):
        require_training_boundary(value, f"v32r3 predecessor {label}")
    ...
    expected_runtime = {
        ...
        "unique_pcm24_topology_refresh": True,
        "candidate_io_topology_pipeline": True,
        "batched_exact_forward_proxy_fft": True,
    ...
    if runtime != expected_runtime:
        raise ValueError("v32r4 runtime implementation contract drift")
    ...
    if scientific != {
        ...
        raise ValueError("v32r4 scientific contract drift")
    ...
    if immutable != expected_immutable:
        raise ValueError("v32r4 immutable boundary drift")
...
def materialize_runtime_candidate(
    ...
    row, timing = v32r3.materialize_runtime_candidate(context, entry, values)
    stored = np.asarray(row["_stored_waveform"], dtype=np.float32)
...
def materialize_and_refresh_candidate(
    context: dict[str, Any],
    entry: dict[str, Any],
    values: np.ndarray,
    ...
    row, timing = materialize_runtime_candidate(context, entry, values)
    materialize_ms = 1000.0 * (time.perf_counter() - started)
    topology_started = time.perf_counter()
    rows, request_ms, staging_rows = worker.refresh_current_waveforms(
        [current_topology_item(entry)],
        [row["_stored_waveform"]],
        NUMPY_HIGHPASS_MODE,
    )
    topology_wall_ms = 1000.0 * (time.perf_counter() - topology_started)
    if len(rows) != 1 or len(staging_rows) != 1:
        raise ValueError("v32r4 single-candidate topology coverage drift")
    return (
        row,
        timing,
        dict(rows[0]),
        ...
        topology_wall_ms,
        float(request_ms),
        float(staging_rows[0]["staging_ms"]),
    )
...
def candidate_layout() -> tuple[list[int], int, int]:
    ...
    if len(unique_indices) != TOPOLOGY_WORKER_COUNT:
        raise ValueError("v32r4 topology worker/candidate layout drift")
...
def batched_metric_pcm16(waveforms: torch.Tensor) -> torch.Tensor:
    if waveforms.ndim != 2 or waveforms.shape[0] == 0:
        raise ValueError("v32r4 proxy batch must be nonempty and two-dimensional")
...
def batched_topology_proxy_values(
    ...
    if len(waveforms) != len(current_topologies) or not waveforms:
        raise ValueError("v32r4 proxy/topology batch coverage drift")
    lengths = {int(np.asarray(waveform).size) for waveform in waveforms}
    if len(lengths) != 1:
        raise ValueError("v32r4 proxy batch waveform length drift")
    ...
    with torch.inference_mode():
        metric_batch = batched_metric_pcm16(values)
        frozen = [
            ...
            for panel_index, panel_row in enumerate(panel_rows, start=1):
                ...
                pipeline_futures: dict[
                    ...
                    index: executor.submit(
                        materialize_and_refresh_candidate,
                        materialize_context,
                        entries[index],
                        candidate_values[index],
                        worker,
                    )
                    for index, worker in zip(
                        unique_indices,
                        workers,
                        strict=True,
                    )
                ...
                pipeline_results = {
                    index: future.result()
                    for index, future in pipeline_futures.items()
                }
                duplicate_row, duplicate_timing = duplicate_future.result()
                ...
                rows_by_index = {
                    index: result[0]
                    for index, result in pipeline_results.items()
                }
                timings_by_index = {
                    index: result[1]
                    for index, result in pipeline_results.items()
                ...
                rows_by_index[raw_zero] = duplicate_row
                timings_by_index[raw_zero] = duplicate_timing
                if (
                    rows_by_index[projected_zero]["candidate_sha256"]
                    != rows_by_index[raw_zero]["candidate_sha256"]
                ):
                    raise ValueError(
                        f"Candidate-E zero-step PCM24 drift: {case_id}"
                    )
                current_by_index[raw_zero] = dict(
                    current_by_index[projected_zero]
                )
                current_by_index[raw_zero]["id"] = (
                    f"current_topology:{entries[raw_zero]['item_id']}"
                )
                case_rows = [
                    rows_by_index[index] for index in range(len(entries))
                ]
                current_rows = [
                ...
                if unique_sha_count != TOPOLOGY_WORKER_COUNT:
                    raise ValueError(
                        f"Candidate-E unique PCM24 coverage drift: {case_id}"
                    )

                proxy_started = time.perf_counter()
                frozen_values, current_values = batched_topology_proxy_values(
                ...
                materialize_ms_values = [
                    result[3] for result in pipeline_results.values()
                ]
                topology_ms_values = [
                    result[4] for result in pipeline_results.values()
                ]
                request_ms_values = [
                    result[5] for result in pipeline_results.values()
                ]
                staging_ms_values = [
                    result[6] for result in pipeline_results.values()
                ]
                runtime_by_case[case_id] = {
                    ...
                    "candidate_io_topology_pipeline_ms": pipeline_wall_ms,
                    "candidate_unique_pcm24_count": float(unique_sha_count),
                    "candidate_materialize_max_ms": max(
                        materialize_ms_values
                    ),
                    "candidate_current_topology_refresh_max_ms": max(
                        topology_ms_values
                    ),
                    "candidate_batched_frozen_and_current_proxy_ms": proxy_ms,
                    "selector_runtime_per_case_ms": 0.0,
                    "total_metric_step_runtime_ms": total_ms,
                    "worker_request_sum_ms_diagnostic": sum(
                        base_requests + request_ms_values
                    ),
                    "candidate_topology_staging_sum_ms_diagnostic": sum(
                        staging_ms_values
                    ),
...
def validate_sources_and_inputs(
    ...
    bindings = {
        ...
        "v32r3_preexact_no_go_report_sha256": runtime_hashes[
            "v32r3_report"
        ],
        "v32r3_preexact_no_go_receipt_sha256": runtime_hashes[
            "v32r3_receipt"
        ],
        "v32r3_preexact_no_go_preserved": True,
        "v32r4_change_class": (
        ...
        "scientific_stage_mapping": {
            ...
            "v32r4": "v26_selector_and_exact_adjudication",
        },
    }
...
def write_receipt(
    ...
    *,
    ...
    path = args.output_dir / "completion_receipt_v32r4.json"
...
def preexact_no_go(
    ...
    report_path = args.output_dir / "external_svd_report_v32r4.json"
...
def build_external_result_rows(
    ...
    for row in rows:
        row["opened_panel"] = "external_svd_v30r2_v31r2_v32r4"
    return rows
...
def main() -> None:
    ...
    if device.type == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("v32r4 requires an allocated CUDA device")
    ...
    equivalence_path = args.output_dir / "candidate_pool_equivalence_v32r4.json"
    v23.write_json(equivalence_path, equivalence)
    attempts_path = args.output_dir / "candidate_e_attempts_pre_exact_v32r4.csv"
    ...
    selector_path = args.output_dir / "selector_seal_pre_exact_v32r4.json"
    ...
    if len(selected) != EXPECTED_CASES:
        raise ValueError("v32r4 selected candidate coverage drift")
    selector_choice = {
    ...
    for case_id, choice in selected.items():
        ...
        if not isinstance(routing, dict):
            raise ValueError(f"v32r4 selector unexpectedly abstained: {case_id}")
    ...
    if exact_versions != target_versions:
        raise ValueError("v31r2 target/v32r4 exact scorer version drift")
    ...
    results_path = args.output_dir / "external_svd_exact_results_v32r4.csv"
