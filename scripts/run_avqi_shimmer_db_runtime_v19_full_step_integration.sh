#!/bin/bash
# Run the hash-bound full-selector-step deterministic contract probe on opened dev.
set -euo pipefail
...
EVALUATOR_SHA256="${EVALUATOR_SHA256:-4e4630b10e34e25d381e50192821d8a22d811976d56ae28bb2250b444fb0ff9a}"
...
REFERENCE_MODE="${REFERENCE_MODE:-immutable_v18}"
MIGRATION_REVIEW="${MIGRATION_REVIEW:-}"
MIGRATION_REVIEW_SHA256="${MIGRATION_REVIEW_SHA256:-}"
DETERMINISTIC_BASELINE_MANIFEST="${DETERMINISTIC_BASELINE_MANIFEST:-}"
DETERMINISTIC_BASELINE_MANIFEST_SHA256="${DETERMINISTIC_BASELINE_MANIFEST_SHA256:-}"
DETERMINISTIC_BASELINE_RECEIPT="${DETERMINISTIC_BASELINE_RECEIPT:-}"
DETERMINISTIC_BASELINE_RECEIPT_SHA256="${DETERMINISTIC_BASELINE_RECEIPT_SHA256:-}"
EXPECTED_NODE="${EXPECTED_NODE:-gpu30}"
EXPECTED_GPU_UUID="${EXPECTED_GPU_UUID:-GPU-ef12a049-30ff-8430-4dc8-0996ce8cef9f}"
...
case "$REFERENCE_MODE" in
  immutable_v18)
    ...
    DEFAULT_JOB_NAME="avqi-shim-v19-full-step"
    ;;
  deterministic_capture)
    DEFAULT_RUN_ROOT="$SOURCE_ROOT/runs/avqi_route_c_shimmer_db_deterministic_full_step_capture_v22_20260830_01"
    DEFAULT_JOB_NAME="avqi-shim-v22-capture"
    ;;
  deterministic_repeat)
    DEFAULT_RUN_ROOT="$SOURCE_ROOT/runs/avqi_route_c_shimmer_db_deterministic_full_step_repeat_v22_20260830_01"
    DEFAULT_JOB_NAME="avqi-shim-v22-repeat"
    ;;
  *)
    echo "Unsupported REFERENCE_MODE: $REFERENCE_MODE" >&2
    exit 2
    ;;
esac
...
if [[ "$REFERENCE_MODE" == "immutable_v18" ]]; then
  if [[ -n "$MIGRATION_REVIEW" || -n "$MIGRATION_REVIEW_SHA256" ]]; then
    echo "Legacy v18 mode must not receive migration review" >&2
    exit 2
  fi
elif [[ -z "$MIGRATION_REVIEW" || -z "$MIGRATION_REVIEW_SHA256" ]]; then
  echo "Deterministic mode requires MIGRATION_REVIEW and SHA256" >&2
  exit 2
else
  REQUIRED_FILES+=("$MIGRATION_REVIEW")
fi
if [[ "$REFERENCE_MODE" == "deterministic_repeat" ]]; then
  if [[ -z "$DETERMINISTIC_BASELINE_MANIFEST" || -z "$DETERMINISTIC_BASELINE_MANIFEST_SHA256" || -z "$DETERMINISTIC_BASELINE_RECEIPT" || -z "$DETERMINISTIC_BASELINE_RECEIPT_SHA256" ]]; then
    echo "Deterministic repeat requires bound baseline manifest and receipt" >&2
    exit 2
  fi
  REQUIRED_FILES+=(
    "$DETERMINISTIC_BASELINE_MANIFEST"
    "$DETERMINISTIC_BASELINE_RECEIPT"
  )
elif [[ -n "$DETERMINISTIC_BASELINE_MANIFEST" || -n "$DETERMINISTIC_BASELINE_MANIFEST_SHA256" || -n "$DETERMINISTIC_BASELINE_RECEIPT" || -n "$DETERMINISTIC_BASELINE_RECEIPT_SHA256" ]]; then
  echo "Only deterministic repeat may receive a prior baseline" >&2
  exit 2
fi
...
if [[ "$REFERENCE_MODE" != "immutable_v18" ]]; then
  verify_sha256 "$MIGRATION_REVIEW" "$MIGRATION_REVIEW_SHA256" "migration review"
fi
if [[ "$REFERENCE_MODE" == "deterministic_repeat" ]]; then
  verify_sha256 "$DETERMINISTIC_BASELINE_MANIFEST" "$DETERMINISTIC_BASELINE_MANIFEST_SHA256" "deterministic baseline manifest"
  verify_sha256 "$DETERMINISTIC_BASELINE_RECEIPT" "$DETERMINISTIC_BASELINE_RECEIPT_SHA256" "deterministic baseline receipt"
fi
...
export REFERENCE_MODE MIGRATION_REVIEW MIGRATION_REVIEW_SHA256
export DETERMINISTIC_BASELINE_MANIFEST DETERMINISTIC_BASELINE_MANIFEST_SHA256
export DETERMINISTIC_BASELINE_RECEIPT DETERMINISTIC_BASELINE_RECEIPT_SHA256
export EXPECTED_NODE EXPECTED_GPU_UUID
if [[ "$REFERENCE_MODE" != "immutable_v18" ]]; then
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ -n "$ACTIVE_JOB_IDS" ]]; then
    echo "Refusing duplicate full-step integration job: $ACTIVE_JOB_IDS" >&2
    exit 2
  fi
  sbatch \
    --parsable \
...
if [[ "$REFERENCE_MODE" != "immutable_v18" ]]; then
  if [[ "${SLURMD_NODENAME:-}" != "$EXPECTED_NODE" ]]; then
    echo "Deterministic node drift: ${SLURMD_NODENAME:-unset}" >&2
    exit 2
  fi
  OBSERVED_GPU_UUID="$(nvidia-smi --query-gpu=uuid --format=csv,noheader | head -n 1)"
  if [[ "$OBSERVED_GPU_UUID" != "$EXPECTED_GPU_UUID" ]]; then
    echo "Deterministic GPU UUID drift: $OBSERVED_GPU_UUID" >&2
    exit 2
  fi
  export OBSERVED_GPU_UUID
fi
...
REFERENCE_ARGS=(
  --candidate-reference-mode "$REFERENCE_MODE"
)
if [[ "$REFERENCE_MODE" != "immutable_v18" ]]; then
  REFERENCE_ARGS+=(
...
if [[ "$REFERENCE_MODE" == "deterministic_repeat" ]]; then
  REFERENCE_ARGS+=(
    --deterministic-baseline-manifest "$DETERMINISTIC_BASELINE_MANIFEST"
    --deterministic-baseline-manifest-sha256 "$DETERMINISTIC_BASELINE_MANIFEST_SHA256"
    --deterministic-baseline-receipt "$DETERMINISTIC_BASELINE_RECEIPT"
    --deterministic-baseline-receipt-sha256 "$DETERMINISTIC_BASELINE_RECEIPT_SHA256"
  )
fi
python "$EVALUATOR" \
