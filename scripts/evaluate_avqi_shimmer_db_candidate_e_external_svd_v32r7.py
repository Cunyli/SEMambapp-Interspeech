...
V32r7 is a pure pre-exact implementation successor of v32r6. It preserves
...
REPORT_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-exact-promotion-v32r7"
)
RECEIPT_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-receipt-v32r7"
)
SELECTOR_SEAL_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-selector-seal-v32r7"
)
PASS_DECISION = "PASS_CANDIDATE_E_EXACT_PRAAT_EXTERNAL_SVD_PROMOTION_V32R7"
PREEXACT_NO_GO = "NO_GO_CANDIDATE_E_EXTERNAL_SVD_PREEXACT_V32R7"
EXACT_NO_GO = "NO_GO_CANDIDATE_E_EXTERNAL_SVD_EXACT_PROMOTION_V32R7"
...
RUNTIME_CONFIG_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-runtime-successor-v32r7"
)
...
def repository_provenance(args: argparse.Namespace) -> dict[str, str]:
    ...
    if root != Path(__file__).resolve().parents[1]:
        raise ValueError("repository root does not contain v32r7 runner")
    head = v23.git_output(root, "rev-parse", "HEAD")
    if head != args.source_commit:
        raise ValueError("v32r7 repository HEAD/source commit drift")
    ...
    if status:
        raise ValueError("v32r7 adjudication requires a clean repository")
    return {
...
def validate_runtime_successor(
    ...
    observed = {
        "runtime_config": v23.validate_hash(
            ...
            "v32r7 runtime successor config",
        ),
        "v32r6_report": v23.validate_hash(
            args.v32r6_report,
            args.v32r6_report_sha256,
            "v32r6 pre-exact NO_GO report",
        ),
        "v32r6_receipt": v23.validate_hash(
            args.v32r6_receipt,
            args.v32r6_receipt_sha256,
            "v32r6 pre-exact NO_GO receipt",
        ),
    }
    config = v23.read_json(args.runtime_config)
    report = v23.read_json(args.v32r6_report)
    receipt = v23.read_json(args.v32r6_receipt)
    if config.get("schema_version") != RUNTIME_CONFIG_SCHEMA:
        raise ValueError("v32r7 runtime config schema drift")
    predecessor = config.get("predecessor_no_go", {})
    expected_predecessor = {
        "job_id": "20043947",
        "state": "COMPLETED",
        "exit_code": "0:0",
        "scientific_decision": v32r6.PREEXACT_NO_GO,
        "source_commit": "f16413e8dc05751af5ada78e40b4e65468710c7f",
        "report_sha256": observed["v32r6_report"],
        "receipt_sha256": observed["v32r6_receipt"],
        "selector_seal_sha256": (
            "dd77821d7e8364d71a79d353e4d77e9dda937864e34717827348c36fa68fe063"
        ),
        "candidate_attempts_sha256": (
            "5c5a06e8e3fec0aa1e0aba1af5dbf422520d857ae8a3bda584e70f35f8715840"
        ),
        "candidate_pool_equivalence_sha256": (
            "a2d6cfc5dbff0e1d0eed74b2daa119b247fbd59fb583abcf2989918991f2a4a6"
        ),
        "stdout_sha256": (
            "af39adf8e13b19f80c89356e6c041ff80b5c66005c5b1b584406d51384e218e9"
        ),
        "stderr_sha256": (
        ...
        "failed_case_count": 1,
        "maximum_total_metric_step_runtime_ms": 547.3849850241095,
        "old_receipt_remains_immutable": True,
        "no_go_not_reinterpreted_as_pass": True,
    }
    if predecessor != expected_predecessor:
        raise ValueError("v32r7 predecessor NO_GO binding drift")
    if report.get("schema_version") != v32r6.REPORT_SCHEMA:
        raise ValueError("v32r6 predecessor report schema drift")
    if report.get("decision") != v32r6.PREEXACT_NO_GO:
        raise ValueError("v32r6 predecessor is not pre-exact NO_GO")
    if report.get("source_commit") != predecessor["source_commit"]:
        raise ValueError("v32r6 predecessor source binding drift")
    if report.get("slurm_job_id") != predecessor["job_id"]:
        raise ValueError("v32r6 predecessor job binding drift")
    expected_gates = {
    ...
    if report.get("preexact_gates") != expected_gates:
        raise ValueError("v32r6 predecessor failure-class drift")
    ...
    if (
        ...
        raise ValueError("v32r6 predecessor runtime evidence drift")
    if (
        ...
        raise ValueError("v32r6 predecessor opened or promoted exact outcomes")
    if receipt.get("schema_version") != v32r6.RECEIPT_SCHEMA:
        raise ValueError("v32r6 predecessor receipt schema drift")
    if receipt.get("decision") != v32r6.PREEXACT_NO_GO:
        raise ValueError("v32r6 predecessor receipt decision drift")
    expected_artifacts = {
        "external_svd_report_v32r6.json": observed["v32r6_report"],
        "selector_seal_pre_exact_v32r6.json": predecessor[
            "selector_seal_sha256"
        ],
        "candidate_e_attempts_pre_exact_v32r6.csv": predecessor[
            "candidate_attempts_sha256"
        ],
        "candidate_pool_equivalence_v32r6.json": predecessor[
            "candidate_pool_equivalence_sha256"
        ],
    }
    if receipt.get("artifact_sha256") != expected_artifacts:
        raise ValueError("v32r6 predecessor receipt artifact binding drift")
    if (
        ...
        raise ValueError("v32r6 predecessor receipt exact-opening drift")
    for label, value in (("report", report), ("receipt", receipt)):
        require_training_boundary(value, f"v32r6 predecessor {label}")
    ...
    expected_runtime_evidence = {
        ...
        "failed_case_count": 1,
        "maximum_total_metric_step_runtime_ms": 547.3849850241095,
        "sole_failed_case_proxy_reduction_after_topology_ms": (
            40.71667406242341
        ),
    ...
    if runtime_evidence != expected_runtime_evidence:
        raise ValueError("v32r7 result-blind runtime evidence binding drift")
    if (
        len(failed_runtime_rows) != 1
        or not math.isclose(
            float(
                failed_runtime_rows[0][
        ...
        raise ValueError("v32r6 predecessor optimization evidence drift")
    ...
    if runtime != expected_runtime:
        raise ValueError("v32r7 runtime implementation contract drift")
    ...
    if scientific != {
        ...
        raise ValueError("v32r7 scientific contract drift")
    ...
    if immutable != expected_immutable:
        raise ValueError("v32r7 immutable boundary drift")
...
def prepare_runtime_candidate(
    ...
    if (
        ...
        raise ValueError("v32r7 in-memory PCM24 waveform shape drift")
...
def write_prepared_runtime_candidate(row: dict[str, Any]) -> float:
    ...
    if not isinstance(encoded, bytes):
        raise ValueError("v32r7 prepared candidate WAV payload is missing")
...
def candidate_layout() -> tuple[list[int], int, int]:
    ...
    if len(unique_indices) != TOPOLOGY_WORKER_COUNT:
        raise ValueError("v32r7 topology worker/candidate layout drift")
...
def batched_metric_pcm16(waveforms: torch.Tensor) -> torch.Tensor:
    if waveforms.ndim != 2 or waveforms.shape[0] == 0:
        raise ValueError("v32r7 proxy batch must be nonempty and two-dimensional")
...
def batched_metric_pcm16_from_waveforms(
    ...
    if not waveforms:
        raise ValueError("v32r7 proxy batch must be nonempty")
    lengths = {int(np.asarray(waveform).size) for waveform in waveforms}
    if len(lengths) != 1:
        raise ValueError("v32r7 proxy batch waveform length drift")
...
def topology_proxy_values_from_metric_batch(
    ...
    if metric_batch.ndim != 2 or metric_batch.shape[0] != len(
        ...
        raise ValueError("v32r7 metric/topology batch coverage drift")
    with torch.inference_mode():
        frozen_tensors = [
...
def batched_topology_proxy_values(
    ...
    if len(waveforms) != len(current_topologies):
        raise ValueError("v32r7 proxy/topology batch coverage drift")
...
def validate_sources_and_inputs(
    ...
    bindings = {
        ...
        "v32r6_preexact_no_go_report_sha256": runtime_hashes[
            "v32r6_report"
        ],
        "v32r6_preexact_no_go_receipt_sha256": runtime_hashes[
            "v32r6_receipt"
        ],
        "v32r6_preexact_no_go_preserved": True,
        "v32r7_change_class": (
        ...
        "scientific_stage_mapping": {
            ...
            "v32r7": "v26_selector_and_exact_adjudication",
        },
    }
...
def write_receipt(
    ...
    *,
    ...
    path = args.output_dir / "completion_receipt_v32r7.json"
...
def preexact_no_go(
    ...
    report_path = args.output_dir / "external_svd_report_v32r7.json"
...
def build_external_result_rows(
    ...
    for row in rows:
        row["opened_panel"] = "external_svd_v30r2_v31r2_v32r7"
    return rows
...
def main() -> None:
    ...
    if device.type == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("v32r7 requires an allocated CUDA device")
    ...
    equivalence_path = args.output_dir / "candidate_pool_equivalence_v32r7.json"
    v23.write_json(equivalence_path, equivalence)
    attempts_path = args.output_dir / "candidate_e_attempts_pre_exact_v32r7.csv"
    ...
    selector_path = args.output_dir / "selector_seal_pre_exact_v32r7.json"
    ...
    if len(selected) != EXPECTED_CASES:
        raise ValueError("v32r7 selected candidate coverage drift")
    selector_choice = {
    ...
    for case_id, choice in selected.items():
        ...
        if not isinstance(routing, dict):
            raise ValueError(f"v32r7 selector unexpectedly abstained: {case_id}")
    ...
    if exact_versions != target_versions:
        raise ValueError("v31r2 target/v32r7 exact scorer version drift")
    ...
    results_path = args.output_dir / "external_svd_exact_results_v32r7.csv"
