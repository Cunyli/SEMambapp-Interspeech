#!/bin/bash
# Profile six CS exact-topology refreshes and one-refresh alpha-grid reuse.
set -euo pipefail
...
PYTHON_SCRIPT="$SOURCE_ROOT/scripts/profile_avqi_shimmer_exact_topology_runtime.py"

RUN_ROOT="${RUN_ROOT:-$SOURCE_ROOT/runs/avqi_route_c_shimmer_runtime_profile_v12_20260824_01}"
...
PANEL_CONTRACT_SHA256="${PANEL_CONTRACT_SHA256:-c831363a1db89655a388d106e779a58a65f1246d1da2156b84d1e086139a28bf}"
CALIBRATION_RESULTS_SHA256="${CALIBRATION_RESULTS_SHA256:-65b66702a24232af0f7a24d68f24eb8ab540ee3411e6810262dc4831b09bebf8}"
FINAL_RESULTS_SHA256="${FINAL_RESULTS_SHA256:-a9808f108a39a29b37f0b1ced4ddc957f59c541a7f08cd15a3028b460b5945fc}"
...
for path in "$PYTHON_SCRIPT" "$PANEL_CONTRACT" "$CALIBRATION_RESULTS" \
  ...
  if [[ ! -f "$path" ]]; then
    echo "Missing Shimmer runtime-profile input: $path" >&2
    exit 2
  fi
done
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  ...
  if [[ -e "$OUTPUT_DIR" ]]; then
    echo "Refusing to overwrite Shimmer runtime-profile output: $OUTPUT_DIR" >&2
    exit 2
  fi
  sbatch \
    --parsable \
    --job-name=avqi-shim-prof-v12 \
    --partition="$PARTITION" \
    --nodes=1 \
    --ntasks=1 \
    --gres="gpu:${GPU_TYPE}:1" \
...
if [[ "$(git -C "$SOURCE_ROOT" rev-parse HEAD)" != "$SOURCE_COMMIT" ]]; then
  echo "Shimmer runtime-profile source HEAD drifted after submission" >&2
  exit 2
fi
if [[ -e "$OUTPUT_DIR" ]]; then
  echo "Refusing to overwrite Shimmer runtime-profile output: $OUTPUT_DIR" >&2
...
LIVE_LOG="$LOG_DIR/shimmer_runtime_profile_${SLURM_JOB_ID}.log"
