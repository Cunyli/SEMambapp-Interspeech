...
V32r8 is a pure pre-exact execution-layout successor of v32r7. It preserves
...
from scripts import evaluate_avqi_shimmer_db_candidate_e_external_svd_v32r6 as v32r6
from scripts import evaluate_avqi_shimmer_db_candidate_e_external_svd_v32r7 as v32r7
...
SELECTOR_SEAL_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-selector-seal-v32r8"
)
PASS_DECISION = "PASS_CANDIDATE_E_EXACT_PRAAT_EXTERNAL_SVD_PROMOTION_V32R8"
PREEXACT_NO_GO = "NO_GO_CANDIDATE_E_EXTERNAL_SVD_PREEXACT_V32R8"
EXACT_NO_GO = "NO_GO_CANDIDATE_E_EXTERNAL_SVD_EXACT_PROMOTION_V32R8"
...
RUNTIME_CONFIG_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-runtime-successor-v32r8"
)
...
def repository_provenance(args: argparse.Namespace) -> dict[str, str]:
    ...
    if root != Path(__file__).resolve().parents[1]:
        raise ValueError("repository root does not contain v32r8 runner")
    head = v23.git_output(root, "rev-parse", "HEAD")
    if head != args.source_commit:
        raise ValueError("v32r8 repository HEAD/source commit drift")
    ...
    if status:
        raise ValueError("v32r8 adjudication requires a clean repository")
    return {
...
def validate_runtime_successor(
    ...
    observed = {
        "runtime_config": v23.validate_hash(
            ...
            "v32r8 runtime successor config",
        ),
        "v32r7_report": v23.validate_hash(
            args.v32r7_report,
            args.v32r7_report_sha256,
            "v32r7 pre-exact NO_GO report",
        ),
        "v32r7_receipt": v23.validate_hash(
            args.v32r7_receipt,
            args.v32r7_receipt_sha256,
            "v32r7 pre-exact NO_GO receipt",
        ),
    }
    config = v23.read_json(args.runtime_config)
    report = v23.read_json(args.v32r7_report)
    receipt = v23.read_json(args.v32r7_receipt)
    if config.get("schema_version") != RUNTIME_CONFIG_SCHEMA:
        raise ValueError("v32r8 runtime config schema drift")
    predecessor = config.get("predecessor_no_go", {})
    expected_predecessor = {
        "job_id": "20044276",
        "state": "COMPLETED",
        "exit_code": "0:0",
        "scientific_decision": v32r7.PREEXACT_NO_GO,
        "source_commit": "6a1254eaaaa6fecde6b058403350051b5011ca1e",
        "report_sha256": observed["v32r7_report"],
        "receipt_sha256": observed["v32r7_receipt"],
        "selector_seal_sha256": (
            "7b7a4258b3065211820414645812bbc7fdae09cc6ead7aedc9d83ec8055532a6"
        ),
        "candidate_attempts_sha256": (
            "43fd7ea0143a431d643a90f960a72125cbcc9915f94521505795642d12041a9d"
        ),
        "candidate_pool_equivalence_sha256": (
            "a2d6cfc5dbff0e1d0eed74b2daa119b247fbd59fb583abcf2989918991f2a4a6"
        ),
        "stdout_sha256": (
            "1027b4dc19c814338879210ae92667658d756488f33d606e6e1dfd96866437b4"
        ),
        "stderr_sha256": (
        ...
        "failed_case_count": 2,
        "maximum_total_metric_step_runtime_ms": 573.7703611375764,
        "old_receipt_remains_immutable": True,
        "no_go_not_reinterpreted_as_pass": True,
    }
    if predecessor != expected_predecessor:
        raise ValueError("v32r8 predecessor NO_GO binding drift")
    if report.get("schema_version") != v32r7.REPORT_SCHEMA:
        raise ValueError("v32r7 predecessor report schema drift")
    if report.get("decision") != v32r7.PREEXACT_NO_GO:
        raise ValueError("v32r7 predecessor is not pre-exact NO_GO")
    if report.get("source_commit") != predecessor["source_commit"]:
        raise ValueError("v32r7 predecessor source binding drift")
    if report.get("slurm_job_id") != predecessor["job_id"]:
        raise ValueError("v32r7 predecessor job binding drift")
    expected_gates = {
    ...
    if report.get("preexact_gates") != expected_gates:
        raise ValueError("v32r7 predecessor failure-class drift")
    ...
    if (
        ...
        raise ValueError("v32r7 predecessor runtime evidence drift")
    if (
        ...
        raise ValueError("v32r7 predecessor opened or promoted exact outcomes")
    if receipt.get("schema_version") != v32r7.RECEIPT_SCHEMA:
        raise ValueError("v32r7 predecessor receipt schema drift")
    if receipt.get("decision") != v32r7.PREEXACT_NO_GO:
        raise ValueError("v32r7 predecessor receipt decision drift")
    expected_artifacts = {
        "external_svd_report_v32r7.json": observed["v32r7_report"],
        "selector_seal_pre_exact_v32r7.json": predecessor[
            "selector_seal_sha256"
        ],
        "candidate_e_attempts_pre_exact_v32r7.csv": predecessor[
            "candidate_attempts_sha256"
        ],
        "candidate_pool_equivalence_v32r7.json": predecessor[
            "candidate_pool_equivalence_sha256"
        ],
    }
    if receipt.get("artifact_sha256") != expected_artifacts:
        raise ValueError("v32r7 predecessor receipt artifact binding drift")
    if (
        ...
        raise ValueError("v32r7 predecessor receipt exact-opening drift")
    for label, value in (("report", report), ("receipt", receipt)):
        require_training_boundary(value, f"v32r7 predecessor {label}")

    failed_runtime_rows = [
        row
        for row in runtimes.values()
        if float(row["total_metric_step_runtime_ms"])
        > hybrid.CACHE_RUNTIME_MAX_MS
    ]
    runtime_evidence = config.get("result_blind_runtime_evidence", {})
    expected_runtime_evidence = {
        ...
        "failed_case_count": 2,
        "maximum_total_metric_step_runtime_ms": 573.7703611375764,
        "maximum_case_base_topology_refresh_ms": 160.68612597882748,
        "maximum_case_candidate_topology_and_metric_overlap_ms": (
            280.4774100659415
        ),
        "predecessor_allocated_cpu_count": 10,
        "predecessor_node_threads_per_core": 2,
        "expected_unique_pcm24_count": EXPECTED_UNIQUE_PCM24_COUNT,
    ...
    if runtime_evidence != expected_runtime_evidence:
        raise ValueError("v32r8 result-blind runtime evidence binding drift")
    if (
        len(failed_runtime_rows) != 2
        or not math.isclose(
            max(
                float(row["base_topology_refresh_ms"])
                for row in runtimes.values()
            ),
            expected_runtime_evidence[
                "maximum_case_base_topology_refresh_ms"
            ],
            rel_tol=0.0,
            abs_tol=1e-12,
        )
        or not math.isclose(
            max(
                float(row["candidate_topology_and_metric_overlap_ms"])
                for row in runtimes.values()
            ),
            expected_runtime_evidence[
                "maximum_case_candidate_topology_and_metric_overlap_ms"
        ...
        or any(
            int(row["candidate_unique_pcm24_count"])
            != EXPECTED_UNIQUE_PCM24_COUNT
            for row in runtimes.values()
        )
    ):
        raise ValueError("v32r7 predecessor runtime evidence drift")
    ...
    expected_runtime = {
        ...
        "unique_pcm24_preparation_count": EXPECTED_UNIQUE_PCM24_COUNT,
        "zero_step_preparation_deduplicated_by_pcm24_identity": True,
    ...
    if scientific != {
        ...
        raise ValueError("v32r8 scientific contract drift")
    ...
    expected_immutable = {
        ...
        "v32r6_preexact_no_go_preserved": True,
    ...
    if immutable != expected_immutable:
        raise ValueError("v32r8 immutable boundary drift")
...
def prepare_runtime_candidate(
    ...
    if (
        ...
        raise ValueError("v32r8 in-memory PCM24 waveform shape drift")
...
def write_prepared_runtime_candidate(row: dict[str, Any]) -> float:
    ...
    if not isinstance(encoded, bytes):
        raise ValueError("v32r8 prepared candidate WAV payload is missing")
...
def candidate_layout() -> tuple[list[int], int, int]:
    ...
    if len(unique_indices) != TOPOLOGY_WORKER_COUNT:
        raise ValueError("v32r8 topology worker/candidate layout drift")
...
def batched_metric_pcm16(waveforms: torch.Tensor) -> torch.Tensor:
    if waveforms.ndim != 2 or waveforms.shape[0] == 0:
        raise ValueError("v32r8 proxy batch must be nonempty and two-dimensional")
...
def proxy_tensor_from_metric_pcm16(
...
def proxy_from_metric_pcm16(
    ...
    return float(
        proxy_tensor_from_metric_pcm16(
            metric_pcm16_full,
            topology,
        )
        .detach()
...
def batched_metric_pcm16_from_waveforms(
    ...
    if not waveforms:
        raise ValueError("v32r8 proxy batch must be nonempty")
    lengths = {int(np.asarray(waveform).size) for waveform in waveforms}
    if len(lengths) != 1:
        raise ValueError("v32r8 proxy batch waveform length drift")
...
def topology_proxy_values_from_metric_batch(
    ...
    if metric_batch.ndim != 2 or metric_batch.shape[0] != len(
        ...
        raise ValueError("v32r8 metric/topology batch coverage drift")
    with torch.inference_mode():
        frozen_tensors = [
            proxy_tensor_from_metric_pcm16(metric, frozen_topology)
            for metric in metric_batch
        ]
        current_tensors = [
            proxy_tensor_from_metric_pcm16(metric, topology)
        ...
        combined = torch.stack([*frozen_tensors, *current_tensors])
        values = combined.detach().cpu().tolist()
    split = int(metric_batch.shape[0])
    frozen = [float(value) for value in values[:split]]
    current = [float(value) for value in values[split:]]
...
def batched_topology_proxy_values(
    ...
    if len(waveforms) != len(current_topologies):
        raise ValueError("v32r8 proxy/topology batch coverage drift")
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=EXECUTOR_WORKER_COUNT) as executor:
            for panel_index, panel_row in enumerate(panel_rows, start=1):
                ...
                if len(prepare_indices) != EXPECTED_UNIQUE_PCM24_COUNT:
                    raise ValueError(
                        f"Candidate-E unique preparation drift: {case_id}"
                    )
                prepare_futures = {
                    ...
                    for index in prepare_indices
                }
                materialized_by_index = {
                    index: future.result()
                    for index, future in prepare_futures.items()
                }
                projected_zero_row, _ = materialized_by_index[projected_zero]
                raw_zero_row = {
                    **projected_zero_row,
                    **{
                        key: entries[raw_zero][key]
                        for key in (
                            "item_id",
                            "case_id",
                            "view",
                ...
                materialized_by_index[raw_zero] = (
                    raw_zero_row,
                    {
                        "pcm24_in_memory_encode_ms": 0.0,
                        "pcm24_memory_decode_and_certificates_ms": 0.0,
                    },
                )
                ...
                runtime_by_case[case_id] = {
                    ...
                    "candidate_unique_pcm24_preparation_count": float(
                        len(prepare_indices)
                    ),
...
def validate_sources_and_inputs(
    ...
    bindings = {
        ...
        "v32r5_preexact_no_go_preserved": True,
        "v32r6_preexact_no_go_decision": v32r6.PREEXACT_NO_GO,
        "v32r6_preexact_no_go_job_id": "20043947",
        "v32r6_preexact_no_go_preserved": True,
        "v32r7_preexact_no_go_decision": v32r7.PREEXACT_NO_GO,
        "v32r7_preexact_no_go_job_id": "20044276",
        "v32r7_preexact_no_go_report_sha256": runtime_hashes[
            "v32r7_report"
        ],
        "v32r7_preexact_no_go_receipt_sha256": runtime_hashes[
            "v32r7_receipt"
        ],
        "v32r7_preexact_no_go_preserved": True,
        "v32r8_change_class": (
        ...
        "scientific_stage_mapping": {
            ...
            "v32r8": "v26_selector_and_exact_adjudication",
        },
    }
...
def write_receipt(
    ...
    *,
    ...
    path = args.output_dir / "completion_receipt_v32r8.json"
...
def preexact_no_go(
    ...
    report_path = args.output_dir / "external_svd_report_v32r8.json"
...
def build_external_result_rows(
    ...
    for row in rows:
        row["opened_panel"] = "external_svd_v30r2_v31r2_v32r8"
    return rows
...
def main() -> None:
    args = parse_args()
    physical_cpu_allocation_certificate()
    ...
    if device.type == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("v32r8 requires an allocated CUDA device")
    ...
    equivalence_path = args.output_dir / "candidate_pool_equivalence_v32r8.json"
    v23.write_json(equivalence_path, equivalence)
    attempts_path = args.output_dir / "candidate_e_attempts_pre_exact_v32r8.csv"
    ...
    selector_path = args.output_dir / "selector_seal_pre_exact_v32r8.json"
    ...
    if len(selected) != EXPECTED_CASES:
        raise ValueError("v32r8 selected candidate coverage drift")
    selector_choice = {
    ...
    for case_id, choice in selected.items():
        ...
        if not isinstance(routing, dict):
            raise ValueError(f"v32r8 selector unexpectedly abstained: {case_id}")
    ...
    if exact_versions != target_versions:
        raise ValueError("v31r2 target/v32r8 exact scorer version drift")
    ...
    results_path = args.output_dir / "external_svd_exact_results_v32r8.csv"
