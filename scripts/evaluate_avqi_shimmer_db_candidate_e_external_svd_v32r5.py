#!/usr/bin/env python3
"""Run frozen Candidate-E external SVD with byte-stable PCM24 materialization.

V32r5 is a pure pre-exact implementation successor of v32r4.  It preserves
...
REPORT_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-exact-promotion-v32r5"
)
RECEIPT_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-receipt-v32r5"
)
SELECTOR_SEAL_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-selector-seal-v32r5"
)
PASS_DECISION = "PASS_CANDIDATE_E_EXACT_PRAAT_EXTERNAL_SVD_PROMOTION_V32R5"
PREEXACT_NO_GO = "NO_GO_CANDIDATE_E_EXTERNAL_SVD_PREEXACT_V32R5"
EXACT_NO_GO = "NO_GO_CANDIDATE_E_EXTERNAL_SVD_EXACT_PROMOTION_V32R5"
...
RUNTIME_CONFIG_SCHEMA = (
    "avqi-route-c-shimmer-db-candidate-e-external-svd-runtime-successor-v32r5"
)
...
def repository_provenance(args: argparse.Namespace) -> dict[str, str]:
    ...
    if root != Path(__file__).resolve().parents[1]:
        raise ValueError("repository root does not contain v32r5 runner")
    head = v23.git_output(root, "rev-parse", "HEAD")
    if head != args.source_commit:
        raise ValueError("v32r5 repository HEAD/source commit drift")
    ...
    if status:
        raise ValueError("v32r5 adjudication requires a clean repository")
    return {
...
def validate_runtime_successor(
    ...
    observed = {
        "runtime_config": v23.validate_hash(
            ...
            "v32r5 runtime successor config",
        ),
        "v32r4_report": v23.validate_hash(
            args.v32r4_report,
            args.v32r4_report_sha256,
            "v32r4 pre-exact NO_GO report",
        ),
        "v32r4_receipt": v23.validate_hash(
            args.v32r4_receipt,
            args.v32r4_receipt_sha256,
            "v32r4 pre-exact NO_GO receipt",
        ),
    }
    config = v23.read_json(args.runtime_config)
    report = v23.read_json(args.v32r4_report)
    receipt = v23.read_json(args.v32r4_receipt)
    if config.get("schema_version") != RUNTIME_CONFIG_SCHEMA:
        raise ValueError("v32r5 runtime config schema drift")
    predecessor = config.get("predecessor_no_go", {})
    expected_predecessor = {
        "job_id": "20043338",
        "state": "COMPLETED",
        "exit_code": "0:0",
        "scientific_decision": v32r4.PREEXACT_NO_GO,
        "source_commit": "318f545aae0ebf3a1c0775f6f51f2e0395ff7553",
        "report_sha256": observed["v32r4_report"],
        "receipt_sha256": observed["v32r4_receipt"],
        "selector_seal_sha256": (
            "1b0fa0b7b929faa53d1e900be3bc64034262ebd26a9d16fdcf3ba4a6f5d7ded3"
        ),
        "candidate_attempts_sha256": (
            "30e2cb56a9377665d66ac654341098572db91c30e53e1c75d1d6134309c02601"
        ),
        "candidate_pool_equivalence_sha256": (
            "a2d6cfc5dbff0e1d0eed74b2daa119b247fbd59fb583abcf2989918991f2a4a6"
        ),
        "stdout_sha256": (
            "ab971e8887723f15cee13123946a2daa1dee148c4f87aafd7431a0744fb63c46"
        ),
        "stderr_sha256": (
        ...
        "failed_case_count": 6,
        "maximum_total_metric_step_runtime_ms": 859.4629579456523,
        "old_receipt_remains_immutable": True,
        "no_go_not_reinterpreted_as_pass": True,
    }
    if predecessor != expected_predecessor:
        raise ValueError("v32r5 predecessor NO_GO binding drift")
    if report.get("schema_version") != v32r4.REPORT_SCHEMA:
        raise ValueError("v32r4 predecessor report schema drift")
    if report.get("decision") != v32r4.PREEXACT_NO_GO:
        raise ValueError("v32r4 predecessor is not pre-exact NO_GO")
    if report.get("source_commit") != predecessor["source_commit"]:
        raise ValueError("v32r4 predecessor source binding drift")
    if report.get("slurm_job_id") != predecessor["job_id"]:
        raise ValueError("v32r4 predecessor job binding drift")
    expected_gates = {
    ...
    if report.get("preexact_gates") != expected_gates:
        raise ValueError("v32r4 predecessor failure-class drift")
    ...
    if (
        ...
        raise ValueError("v32r4 predecessor runtime evidence drift")
    if (
        ...
        raise ValueError("v32r4 predecessor opened or promoted exact outcomes")
    if receipt.get("schema_version") != v32r4.RECEIPT_SCHEMA:
        raise ValueError("v32r4 predecessor receipt schema drift")
    if receipt.get("decision") != v32r4.PREEXACT_NO_GO:
        raise ValueError("v32r4 predecessor receipt decision drift")
    expected_artifacts = {
        "external_svd_report_v32r4.json": observed["v32r4_report"],
    ...
    if receipt.get("artifact_sha256") != expected_artifacts:
        raise ValueError("v32r4 predecessor receipt artifact binding drift")
    if (
        ...
        raise ValueError("v32r4 predecessor receipt exact-opening drift")
    for label, value in (("report", report), ("receipt", receipt)):
        require_training_boundary(value, f"v32r4 predecessor {label}")
    ...
    expected_runtime = {
        ...
        "durable_single_write_without_disk_readback": True,
        "serial_candidate_materialization": True,
        "sealed_pcm24_reencoding_byte_audit": {
            "candidate_count": 120,
            "byte_identical_count": 120,
            "zero_step_exact_input_pcm16_identity_count": 24,
            "candidate_exact_outcomes_used": False,
            "audit_script_sha256": (
                "8346faabd258d3d1f06fd46d2191aa2416c2f571b416fee7669a4184ef82f2a7"
            ),
        },
    ...
    if runtime != expected_runtime:
        raise ValueError("v32r5 runtime implementation contract drift")
    ...
    if scientific != {
        ...
        raise ValueError("v32r5 scientific contract drift")
    ...
    if immutable != expected_immutable:
        raise ValueError("v32r5 immutable boundary drift")
...
def materialize_runtime_candidate(
    ...
    durable_write_ms = 1000.0 * (time.perf_counter() - write_started)
    ...
    if (
        ...
        raise ValueError("v32r5 in-memory PCM24 waveform shape drift")
    ...
    return row, {
        ...
        "pcm24_durable_single_write_ms": durable_write_ms,
...
def candidate_layout() -> tuple[list[int], int, int]:
    ...
    if len(unique_indices) != TOPOLOGY_WORKER_COUNT:
        raise ValueError("v32r5 topology worker/candidate layout drift")
...
def batched_metric_pcm16(waveforms: torch.Tensor) -> torch.Tensor:
    if waveforms.ndim != 2 or waveforms.shape[0] == 0:
        raise ValueError("v32r5 proxy batch must be nonempty and two-dimensional")
...
def batched_topology_proxy_values(
    ...
    if len(waveforms) != len(current_topologies) or not waveforms:
        raise ValueError("v32r5 proxy/topology batch coverage drift")
    lengths = {int(np.asarray(waveform).size) for waveform in waveforms}
    if len(lengths) != 1:
        raise ValueError("v32r5 proxy batch waveform length drift")
        ...
        with ThreadPoolExecutor(max_workers=EXECUTOR_WORKER_COUNT) as executor:
            for panel_index, panel_row in enumerate(panel_rows, start=1):
                ...
                runtime_by_case[case_id] = {
                    ...
                    "candidate_serial_pcm24_materialization_ms": (
                        materialize_wall_ms
                    ),
                    ...
                    "candidate_current_topology_refresh_parallel_ms": (
                        topology_wall_ms
                    ),
                    ...
                    "pcm24_durable_single_write_sum_ms_diagnostic": sum(
                        timing["pcm24_durable_single_write_ms"]
                        for timing in timings_by_index.values()
                    ),
...
def validate_sources_and_inputs(
    ...
    bindings = {
        ...
        "v32r4_preexact_no_go_report_sha256": runtime_hashes[
            "v32r4_report"
        ],
        "v32r4_preexact_no_go_receipt_sha256": runtime_hashes[
            "v32r4_receipt"
        ],
        "v32r4_preexact_no_go_preserved": True,
        "v32r5_change_class": (
        ...
        "scientific_stage_mapping": {
            ...
            "v32r5": "v26_selector_and_exact_adjudication",
        },
    }
...
def write_receipt(
    ...
    *,
    ...
    path = args.output_dir / "completion_receipt_v32r5.json"
...
def preexact_no_go(
    ...
    report_path = args.output_dir / "external_svd_report_v32r5.json"
...
def build_external_result_rows(
    ...
    for row in rows:
        row["opened_panel"] = "external_svd_v30r2_v31r2_v32r5"
    return rows
...
def main() -> None:
    ...
    if device.type == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("v32r5 requires an allocated CUDA device")
    ...
    equivalence_path = args.output_dir / "candidate_pool_equivalence_v32r5.json"
    v23.write_json(equivalence_path, equivalence)
    attempts_path = args.output_dir / "candidate_e_attempts_pre_exact_v32r5.csv"
    ...
    selector_path = args.output_dir / "selector_seal_pre_exact_v32r5.json"
    ...
    if len(selected) != EXPECTED_CASES:
        raise ValueError("v32r5 selected candidate coverage drift")
    selector_choice = {
    ...
    for case_id, choice in selected.items():
        ...
        if not isinstance(routing, dict):
            raise ValueError(f"v32r5 selector unexpectedly abstained: {case_id}")
    ...
    if exact_versions != target_versions:
        raise ValueError("v31r2 target/v32r5 exact scorer version drift")
    ...
    results_path = args.output_dir / "external_svd_exact_results_v32r5.csv"
